...
3. Install PostgreSQL 16 via [this web link](https://www.postgresql.org/download/linux/redhat/ "PGDG Install Instructions for Red Hat Family"), ensuring the correct OL/Rocky version is selected. 

## Setup

1. Connect to the postgreSQL terminal as the linux account postgres:
...
8. access the install scripts from the git repository. You may choose to pull the entire repo down, or you can pull the install scripts and move them to the server. Once done, cd to them and Run the following scripts, to built the Ribbon2 SIS database, as the postgres user:
    ...
    psql -U ribbon2 -d ribbon2 -f 6-helmsman.sql
    ```

9. Connect to ribbon2 database as user ribbon2, as the postgres linux user:
